...
dotenv.config();
const isBuyer = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const token =
    ...
    const existBuyer = await prisma.user.findUnique({
      where: { id: user.user_id, role: "BUYER" },
      include: { sessions: true },
    });

    if (!existBuyer) {
      return clearCookies(res, "Buyer not found");
    }

    const existSession = existBuyer.sessions.find(
